...
namespace Peak.AP
{
    public static class ItemToBombTrapEffect
    {
        public static void ApplyItemToBombTrap(ManualLogSource log)
        {
            try
            {
                ...
                {
                    log.LogWarning("[PeakPelago] Cannot apply Item to Bomb trap - no characters found");
                    return;
                }
                ...
                {
                    log.LogWarning("[PeakPelago] Cannot apply Item to Bomb trap - no characters holding items");
                    return;
                }

                log.LogInfo($"[PeakPelago] Applying Item to Bomb Trap to {validCharacters.Count} character(s)!");

                // Apply to all characters holding items
                foreach (var character in validCharacters)
                {
                    character.StartCoroutine(ReplaceItemWithDynamite(character, log));
                }
            }
            catch (Exception ex)
            {
                log.LogError($"[PeakPelago] Error applying Item to Bomb trap: {ex.Message}");
                log.LogError($"[PeakPelago] Stack trace: {ex.StackTrace}");
            }
        }

        private static IEnumerator ReplaceItemWithDynamite(Character targetCharacter, ManualLogSource log)
        {
            ...
            // Call the RPC directly to spawn dynamite in hand
            characterItems.photonView.RPC("RPC_SpawnItemInHandMaster", RpcTarget.MasterClient, "Dynamite");
            
            log.LogInfo($"[PeakPelago] Replaced {itemName} with Dynamite in slot {currentSlot} for {characterName}!");
        }
    }
}